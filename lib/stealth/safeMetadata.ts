/**
 * SAFE METADATA SYSTEM
 * 
 * Clean, inbox-safe, scanner-friendly, SEO-friendly metadata
 * Rotates through 10+ safe variations to avoid detection patterns
 * 
...
/**
 * Safe metadata rotations (10+ variations)
 * All are inbox-safe, SEO-friendly, scanner-friendly
 */
const SAFE_METADATA_ROTATIONS: SafeMetadata[] = [
  {
...
/**
 * Get safe metadata (rotated)
 * Uses request fingerprint or timestamp to rotate through variations
 */
export function getSafeMetadata(
  ...
  const index = seedValue % SAFE_METADATA_ROTATIONS.length
  return SAFE_METADATA_ROTATIONS[index]
}

/**
 * Get safe metadata for specific domain/company
 * Customizes metadata based on domain while keeping it safe
 */
export function getSafeMetadataForDomain(
  domain: string,
  seed?: string | number
): SafeMetadata {
  const base = getSafeMetadata(seed)
...
/**
 * Generate safe HTML meta tags
 * Returns clean, SEO-friendly meta tags
 */
export function generateSafeMetaTags(metadata: SafeMetadata): string {
  const tags: string[] = []
