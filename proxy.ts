...
// Proxy runs in Edge runtime by default (faster, works with Web Crypto API)
...
async function loadMiddlewareSettings(origin: string): Promise<MiddlewareSettings> {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('[Proxy] Failed to load settings from API:', error)
    }
  }
...
async function logBotDetection(
  ...
  try {
    ...
    await fetch(`${origin}/api/internal/log-visitor`, {
      ...
      // Silent fail - don't block proxy if logging fails
    })
  } catch (error) {
    // Silent fail - don't block proxy if logging fails
  }
...
export async function proxy(request: NextRequest) {
  // ====================================================================
  // STEP 1: MINIMAL DEBUG LOGGING (Proxy debugging ONLY)
  // ====================================================================
  ...
  // Avoid recursive proxy executions when logging visitors
...
if (!ADMIN_PASSWORD) {
  throw new Error('[Proxy] ADMIN_PASSWORD environment variable is required.')
}
  ...
  try {
    ...
    if (scannerDetection.isScanner) {
      if (isLocalhost && process.env.NODE_ENV === 'development') {
        console.warn('[Proxy] Scanner detected but running in localhost test mode; skipping safe-content rewrite.')
      } else {
      if (process.env.NODE_ENV === 'development') {
        console.warn('[Proxy] Scanner detected early:', {
          ip,
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('[Proxy] Scanner detection failed:', error)
      }
    }
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn('[Proxy] Firewall evaluation API failed:', error)
    }
  }
  ...
  if (firewallResult) {
    switch (firewallResult.action) {
      case 'benign': {
        ...
        // Use admin settings for Telegram (from settings object), fallback to .env
        const telegramEnabled = (settings.notifications?.telegram?.enabled !== false) &&
                               (settings.notifications?.telegram?.botToken || process.env.TELEGRAM_BOT_TOKEN) &&
                               (settings.notifications?.telegram?.chatId || process.env.TELEGRAM_CHAT_ID) &&
  ...
  try {
    ...
    console.error('[PROXY] Device detection error:', error)
  }
