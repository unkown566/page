...
/**
 * CLIENT COMPONENT - All client-side logic (React hooks, state, etc.)
 * This component is wrapped by page-server.tsx which validates token server-side
 */
...
function HomeContent() {
  ...
  useEffect(() => {
    const checkLinkStatus = async () => {
      // CRITICAL FIX: Extract token/identifier from pathname first (Format A with short identifier)
      const pathname = typeof window !== 'undefined' ? window.location.pathname : ''
      let token = searchParams.get('token')
      let identifier: string | null = null
      
      // Check if pathname contains /r/ (Format A or C)
      if (pathname.includes('/r/') && !token) {
        const extracted = extractTokenFromPathname(pathname)
        if (extracted) {
          // Check if it's a short identifier (8-12 chars, alphanumeric) or a long token
          const isShortIdentifier = /^[0-9a-zA-Z]{8,12}$/.test(extracted)
          if (isShortIdentifier) {
            // It's a short identifier - pass it separately
            identifier = extracted
            console.log('[FRONTEND] Detected short identifier in pathname:', identifier)
          } else {
            // It's a long token (legacy) - use it as token
            token = extracted
          }
        }
      }
      ...
      // CRITICAL FIX: Allow identifier OR token (Format A can use short identifier)
      if (!token && !identifier) {
      ...
      try {
        ...
        // CRITICAL FIX: Include identifier for Format A short identifier links
        // Backend will validate email against allowlist (Type B) or resolve email from mappingId (Type C)
        // Backend will resolve identifier to token (Format A)
        const response = await fetch(API_ROUTES.checkLinkStatus, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
            token: token || undefined, // Token (for long tokens or legacy)
            identifier: identifier || undefined, // Short identifier (for Format A)
            id,
  ...
  useEffect(() => {
    const checkFirewall = async () => {
      ...
      try {
        ...
        const firewallCheck = await fetch('/api/firewall/check', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    // This useEffect only extracts email for UI, NOT validation
    if (tokenParam && !emailParam && !idParam) {
      fetch(`/api/email-from-token?token=${encodeURIComponent(tokenParam)}`)
        .then(r => {
        ...
        .then(data => {
          ...
          if (data && data.email) {
            console.log('[FRONTEND] Email resolved from token:', {
            ...
            // CRITICAL FIX: Log when email is missing
            console.warn('[FRONTEND] email-from-token returned no email:', {
              hasData: !!data,
        ...
        .catch((error) => {
          ...
          console.error('[FRONTEND] email-from-token fetch error:', {
            error: error instanceof Error ? error.message : String(error),
          })
          // PHASE 7.5: Don't redirect here - let first useEffect handle validation
    ...
    if (mappingIdParam && tokenParam) {
      fetch(`/api/email-from-mapping?mappingId=${encodeURIComponent(mappingIdParam)}`)
        .then(r => r.json())
        .then(data => {
          if (data.email) {
            setEmail(data.email)
